...
class QAdmixModel( object ):
  ...
  def calc_local_params( self, Data, LP=None):
    ...
    LP['Elogw_perGroup'] = digamma( alpha_perGroup ) - digamma( alpha_perGroup.sum(axis=1) )[:,np.newaxis]
    ...
    for gg in range( len(GroupIDs) ):
      LP['N_perGroup'][gg] = np.sum( resp[ GroupIDs[gg] ], axis=0 )
    return LP
  ...
  def E_logqZ( self, GroupIDs, LP ):
    ElogqZ = 0
    for gg in xrange( len(GroupIDs) ):
      ElogqZ += np.sum( LP['resp'][GroupIDs[gg]] * LP['resp'][GroupIDs[gg]] )
    return  ElogqZ    
